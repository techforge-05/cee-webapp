    ...
    <template v-for="section in schema" :key="section.pageKey">
      ...
      <UCard v-if="section.type === 'info'" class="text-center">
        <div class="py-6 flex flex-col items-center gap-3">
          <UIcon
            v-if="section.infoIcon"
            :name="section.infoIcon"
          ...
          <p
            v-if="section.infoDescriptionKey"
            class="text-sm text-gray-500 max-w-md"
          >
        ...
        <div class="space-y-4">
          ...
          <template v-else>
            ...
            <DataListManager
              ...
              :item-label="
                section.itemLabelKey ? $t(section.itemLabelKey) : undefined
              "
                    ...
                    <template v-for="field in section.fields" :key="field.key">
                      ...
                      >
                        <div class="flex items-center gap-2">
                          <USelect
                            v-if="
                              !isSelectCustom(section.pageKey, index, field)
                            "
                            :model-value="item.metadata?.[field.key] || ''"
                            :items="[
                              {
                                value: '__custom__',
                                label: $t('admin.editors.generic.custom'),
                              },
                              ...field.options.map((o) => ({
                                value: o.value,
                                label: $t(o.labelKey),
                          ...
                          <template v-else>
                            <UInput
                              :model-value="item.metadata?.[field.key] || ''"
                              :placeholder="
                                $t('admin.editors.generic.customValue')
                              "
                              class="min-w-48"
                              @update:model-value="
                                updateListMetadataField(
...
<script setup lang="ts">
  ...
  function hasImageField(section: EditorSection): boolean {
    return section.fields.some(
      (f) => f.type === 'metadata' && f.key === 'imageUrl',
    );
  }
    ...
    try {
      for (const section of props.schema) {
        ...
        if (section.type === 'single') {
          ...
          const metaFields = section.fields.filter(
            (f) => f.type === 'metadata' || f.type === 'select',
          );
          listData[section.pageKey] =
      ...
      toast.add({
        title: t('admin.editors.loadError'),
  ...
  function updateSingleIconField(
    pageKey: string,
    fieldKey: string,
    val: string,
  ) {
  ...
  function handleCancel() {
    ...
    const currentUrls = collectUrls([{ ...sectionImages }, { ...listData }])
